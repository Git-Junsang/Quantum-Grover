//==============================================================================
// grover_iteration.v -- hardware_dram branch, reused from hardware_bram v0.7g
//
// Byte-identical copy of grover_iter_datapath + grover_ctrl_fsm from
// hardware_bram/src_v2/grover_iteration.v. Neither module has any coupling to
// the K3/K4 checkpoint mechanism: grover_ctrl_fsm drives a single amp_mem
// (INIT -> PASS1 -> MEAN -> PASS2, one row-streamed iteration at a time) and
// exposes amp_wr_en/amp_wr_row/amp_row_out for whatever memory is wired below
// it. The hardware_dram branch reuses this same controller unmodified as its
// growth engine: grover_dram_amp_store.v snapshots the buffer to DRAM after
// each completed iteration instead of relying on grover_checkpoint.v's K3/K4
// replay planner.
//
// The checkpoint-timing variant grover_ctrl_fsm_ckpt (and its
// grover_ckpt_oracle_align companion) is intentionally NOT copied here: that
// controller exists only to absorb grover_ckpt_mem's extra registered
// source-select cycle, which does not exist in this branch's plain
// grover_amp_mem buffers.
//==============================================================================
