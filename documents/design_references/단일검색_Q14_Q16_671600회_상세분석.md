...
첫째, 이 캠페인은 **v0.7g 세대의 골든 모델**을 돌린 것이다. 지금 정본은 보드에서 검증된 K3/H3-E4-M2(`hardware_bram/src/`)이고, 그 세대의 checkpoint 는 슬롯 3개 + policy 로 바뀌었다. 여기 나오는 `BBHT_BURST` 는 슬롯 하나짜리 옛 구조다. 따라서 이 문서의 **절감률을 현재 하드웨어의 성능으로 인용하면 안 된다.** 현행 성능의 정본은 보드 실측(`hardware_bram/vivado/vivado_bbht_grover_fpga/2026-09-08_k3h3_e4_m2_board_500run/`, 실경과 시간)과 6단계 ablation(`hardware_bram/results/2026-09-08_publication_6stage/`, RTL 사이클)이다.
...
현행 소프트웨어 기준모델과 보드 비교 실험은 [소프트웨어_기준모델과_Common500.md](소프트웨어_기준모델과_Common500.md) 에 있다.
