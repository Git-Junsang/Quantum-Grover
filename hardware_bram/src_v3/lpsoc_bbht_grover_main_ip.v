//==============================================================================
// lpsoc_bbht_grover_main_ip.v -- BBHT/Grover Main IP with K4/H6 checkpoint execution
//
...
module lpsoc_bbht_grover_main_ip #(
    ...
    wire run_auto_mode_legal = !run_ckpt_auto_mode ||
        (run_burst_mode && run_auto_shot && (CKPT_K == 4));
    ...
    generate
        if (CHECKPOINT_ENABLE != 0) begin : g_ckpt_plan_exec
            ...
            if (CKPT_K == 4) begin : g_auto_policy_k4
                ...
                grover_ckpt_policy_rolling u_auto_policy (
                    .clk                  (clk),
                    .rstn                 (rstn),
            ...
            end else begin : g_no_auto_policy_k4
    ...
    //==========================================================================
    // Inner Grover controller + v0.7g datapath
    //==========================================================================
    wire [3:0] iter_state;
    ...
    grover_iter_datapath u_iter_dp (
        ...
        .row_out_is_pass2(dp_row_out_is_pass2)
    );
    ...
    //==========================================================================
    // Born + classical verify, including Enumeration found-mask exclusion.
    //==========================================================================
    wire meas_busy;
    ...
    //==========================================================================
    // v3 PASS2 융합 -- 측정용 512행 재스캔 제거
    //
    // PASS2 가 새 psi_j 의 한 행(32 x 23비트)을 amp_mem 에 쓰는 그 사이클에,
    // 같은 데이터를 제곱기에 흘려 row_weight 를 만듭니다. 그러면 측정이
    // 시작될 때 row_weight 메모리와 총합이 이미 완성되어 있어 S_BUILD 의
    // 512행 스캔이 통째로 사라집니다.
    //
    // 제곱기는 재사용하지 않고 한 벌 더 둡니다. 보드 DSP 가 240개라 여유가
    // 있고, 공유하면 측정 경로와의 중재가 붙어 오히려 복잡해집니다.
    //
    // 유효성:
    //   - PASS2 로 512행을 빠짐없이 채운 뒤에만 fuse_valid 가 섭니다.
    //   - PASS2 가 아닌 amp_mem 쓰기(초기 적재, checkpoint 복원)가 한 번이라도
    //     들어오면 즉시 무효화하고, 그 샷은 기존 S_BUILD 경로로 되돌아갑니다.
    //     j=0 처럼 Grover 반복이 없는 샷이 여기에 해당합니다.
    //==========================================================================
    wire fuse_sq_valid;
    wire [`GP_ROW_W-1:0]  fuse_sq_row;
    wire [`GP_ROW_SUM_W-1:0] fuse_sq_row_sum;

    wire fuse_pass2_commit = amp_mem_wr_en && dp_row_out_is_pass2;
    wire fuse_other_commit = amp_mem_wr_en && !dp_row_out_is_pass2;
    // S_BUILD 폴백 스캔이 row_weight 메모리를 다른 슬롯 값으로 덮어씁니다.
    // 누산기만 보면 총합은 맞는데 행별 가중치가 남의 것이 됩니다.
    wire meas_build_overwrite;

    grover_born_square32 u_fuse_square32 (
        .clk        (clk),
        .rstn       (rstn),
        .in_valid   (fuse_pass2_commit),
        .in_row     (ctrl_amp_wr_row),
        .in_amp     (dp_amp_row_out),
        .out_valid  (fuse_sq_valid),
        .out_row    (fuse_sq_row),
        .out_sq     (),
        .out_row_sum(fuse_sq_row_sum)
    );

    // 제곱기는 4단이라, PASS2 마지막 행의 가중치는 쓰기보다 4클럭 늦게 나옵니다.
    // 그 사이에 측정이 시작되면 512행을 다 못 센 상태로 판정해 버립니다
    // (2026-09-06: Normal 모드가 27샷 전부 이것 때문에 융합을 놓쳤습니다).
    // in-flight 를 추적해서 sampler 가 드레인을 기다리게 합니다.
    reg [5:0] fuse_inflight_sr;
    always @(posedge clk) begin
        if (!rstn) fuse_inflight_sr <= 6'd0;
        else       fuse_inflight_sr <= {fuse_inflight_sr[4:0], fuse_pass2_commit};
    end
    wire fuse_pending = fuse_pass2_commit || (|fuse_inflight_sr);

    reg [`GP_TOTAL_WEIGHT_W-1:0] fuse_total_acc;
    reg [`GP_ROW_W:0]            fuse_row_count;
    reg                          fuse_valid_r;
    reg                          fuse_dirty;
    // 누산한 row_weight 가 "어느 checkpoint 슬롯의 psi" 인지 기억합니다.
    // checkpoint 복원은 amp_mem 에 쓰지 않고 읽는 슬롯만 바꾸므로, 쓰기만
    // 감시하면 슬롯 X 로 만든 가중치를 슬롯 Y 측정에 써 버립니다
    // (2026-09-06: 250쌍 중 59쌍이 이것 때문에 궤적이 어긋났습니다).
    reg [1:0]                    fuse_slot_r;

    wire [`GP_TOTAL_WEIGHT_W-1:0] fuse_sum_ext =
        {{(`GP_TOTAL_WEIGHT_W-`GP_ROW_SUM_W){1'b0}}, fuse_sq_row_sum};

    always @(posedge clk) begin
        if (!rstn) begin
            fuse_total_acc <= {`GP_TOTAL_WEIGHT_W{1'b0}};
            fuse_row_count <= {(`GP_ROW_W+1){1'b0}};
            fuse_valid_r   <= 1'b0;
            fuse_dirty     <= 1'b0;
            fuse_slot_r    <= 2'd0;
        end else begin
            // PASS2 가 아닌 쓰기가 들어오면 지금 모으던 것을 버립니다.
            if (fuse_other_commit || meas_build_overwrite) begin
                fuse_valid_r   <= 1'b0;
                fuse_dirty     <= 1'b1;
                fuse_row_count <= {(`GP_ROW_W+1){1'b0}};
                fuse_total_acc <= {`GP_TOTAL_WEIGHT_W{1'b0}};
            end else if (fuse_sq_valid) begin
                if (fuse_sq_row == {`GP_ROW_W{1'b0}}) begin
                    // 행 0 은 새 스윕의 시작입니다. 누산을 다시 엽니다.
                    fuse_total_acc <= fuse_sum_ext;
                    fuse_row_count <= {{`GP_ROW_W{1'b0}}, 1'b1};
                    fuse_valid_r   <= 1'b0;
                    fuse_dirty     <= 1'b0;
                    fuse_slot_r    <= ckpt_amp_wr_slot;
                end else if (ckpt_amp_wr_slot != fuse_slot_r) begin
                    // 스윕 도중 쓰기 슬롯이 바뀌면 그 누산은 버립니다.
                    fuse_dirty     <= 1'b1;
                    fuse_valid_r   <= 1'b0;
                end else begin
                    fuse_total_acc <= fuse_total_acc + fuse_sum_ext;
                    fuse_row_count <= fuse_row_count + 1'b1;
                    // 512행을 순서대로 다 채웠을 때만 유효합니다.
                    if (!fuse_dirty && (fuse_sq_row == (`GP_ROWS-1)) &&
                        (fuse_row_count == (`GP_ROWS-1)))
                        fuse_valid_r <= 1'b1;
                end
            end
        end
    ...
    grover_measure_verify #(
        ...
        .build_overwrite      (meas_build_overwrite),
        .fuse_pending         (fuse_pending),
        .fuse_valid           (fuse_valid_r && (fuse_slot_r == ckpt_amp_rd_slot)),
        .fuse_total_weight    (fuse_total_acc),
        .fuse_wr_en           (fuse_sq_valid),
        .fuse_wr_row          (fuse_sq_row),
        .fuse_wr_weight       (fuse_sq_row_sum),
    ...
    grover_ckpt_segment_router u_ckpt_segment_router (
        .pass1_active(ckpt_pass1_active),
        .bridge      (ckpt_exec_bridge),
        .src_slot    (ckpt_exec_src_slot),
        .dst_slot    (ckpt_exec_dst_slot),
        .amp_rd_slot (ckpt_iter_rd_slot),
        .amp_wr_slot (ckpt_iter_wr_slot)
    );

    // Measurement always reads the executor endpoint.  During direct legacy/K1
    // checkpoint operation, slot0 remains the only physical state.
    ...
    wire [`GP_ROW_W-1:0] amp_mem_rd_row = meas_busy ? meas_amp_rd_row
                                                     : iter_amp_rd_row;
    wire amp_mem_rd_en = meas_busy ? meas_amp_rd_en : iter_amp_rd_en;
    wire amp_mem_wr_en = ctrl_amp_wr_en && dp_row_out_valid &&
    ...
    generate
        if (CHECKPOINT_ENABLE != 0) begin : g_ckpt_amp_mem
            // Runtime slot ownership is selected by the manual Planner/Executor
            // in Phase 4B.  When manual mode is disabled the verified Phase-4A
            // K1-compatible slot0 path is preserved.
            grover_ckpt_mem #(.CKPT_K(CKPT_K)) u_amp_mem (
                .clk     (clk),
                .rd_slot (ckpt_amp_rd_slot),
                .rd_row  (amp_mem_rd_row),
                .rd_en   (amp_mem_rd_en),
                .rd_amp  (amp_mem_rdata),
                .rd_valid(ckpt_amp_rd_valid),
                .wr_slot (ckpt_amp_wr_slot),
                .wr_row  (ctrl_amp_wr_row),
                .wr_en   (amp_mem_wr_en),
                .wr_amp  (dp_amp_row_out)
            );
    ...
    grover_found_mask_mem u_found_mask (
        .clk                 (clk),
        .rstn                (rstn),
        .oracle_rd_row       (iter_data_rd_row),
        .oracle_rd_en        (iter_data_rd_en),
    ...
    wire _unused_ckpt_oracle_valid = ckpt_oracle_aligned_valid;
