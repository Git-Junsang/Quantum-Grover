...
//
// 통신 계층(bbht_rvx_wrapper)은 인수인계 §3.4 계약 이름 그대로인
// bbht_grover_core 를 인스턴스합니다. PJK 최종 소스의 알맹이는 모듈
// 이름이 lpsoc_bbht_grover_main_ip 이고 리셋 이름이 rstn 이라, 그
// 둘만 맞춰 주는 얇은 껍데기를 둡니다. 신호는 61개 전부 1:1 이고
// 폭 변환도 논리도 없습니다.
//
// 이 파일이 있는 이유:
//   - Main IP 소스는 freeze 대상이라 손대지 않습니다 (최종집약본 16.4)
//   - 우리 wrapper 도 계약 이름을 유지해야 check_ports 가 그대로 돕니다
//   - 그래서 이름·리셋 차이만 여기서 흡수합니다
//
// 파라미터는 최종집약본 16.3 의 production 구성입니다. 이 소스의 policy
// 는 H_FUTURE=4 이므로 실제 동작은 K4/H4 입니다 (grover_policy.v 165행).
// 파일 주석 여기저기에 H6·H7·H8 이 남아 있지만 전부 옛 표기이고,
// 정본은 localparam H_FUTURE / WINDOW_LEN / MEM_ENTRIES 입니다.
//
// src_v2 에서 합성 경로에 들어가는 파일은 아래 열하나입니다.
//   lpsoc_bbht_grover_main_ip.v   최상위
//   grover_iteration.v            데이터패스 · 제어 FSM
//   grover_arithmetic.v           술어 · 가산트리 · 평균 · 확산
//   grover_memories.v             data / amp / found_mask / result FIFO
//   grover_measurement.v          Born 측정 · verify
//   grover_bbht.v                 LFSR · m ROM · shot FSM
//   grover_checkpoint.v           K4 저장 · Planner · Executor
//   grover_policy.v               memo · Shadow-J · plan FIFO
//   grover_status.v               카운터
//   grover_loader.v               Main IP 내부 적재 제어
//   grover_param.vh               헤더
//
// 나머지 여섯은 합성 경로에 넣지 마십시오.
//   bbht_rvx_wrapper.v / bbht_grover_mmio.v / bbht_ahb_loader.v
//       PJK 가 같이 준 통신 계층입니다. 우리 src/ 쪽과 역할이 겹칩니다.
//       CSR 38개 주소와 STATUS 비트 배치를 대조하는 용도로만 둡니다.
//   lpsoc_bbht_grover_timing_wrapper.v
//   grover_policy_ooc_top.v / grover_policy_impl_wrapper.v
//       standalone 타이밍 증명과 OOC 합성 전용입니다. 최상위가 둘이 됩니다.
//=====================================================================
`timescale 1ns/1ps
